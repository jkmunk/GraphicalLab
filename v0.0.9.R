...
transform_add <- function(tmp,pltydsigm,yssigm2,dsigmtrdt,linetype,linecolour,analyses) {
  ...
  pltydsigm <- add_trace(pltydsigm,
    ...
    add_trace(x = c(tmp$time,
              ...
              y = c(tmp$SD + tmp$uptrans,
                    rev(tmp$SD - tmp$dntrans)),
...
# Add d datasets
for (d in 51:nrow(Data)) { # For every analyte,
  ...
  if (!((is.na(analyses[Alistrow,
    ...
    # Transform and add to plot using function transform_add
  ...
  # Else, if only an upper RI is defined, such as for CRP, for which a lower limit makes no sense,
    ...
    for (i in 1:length(tempraw)) {
      ...
      # or if data point is not missing, nor NA,
        ...
        # This may be an analysis of the (0 1) form.
        # If so, handle that...
        # if (grepl(pattern = "0 1", x = analyses[Alistrow,1])) {
        #   tmp <- rbind(tmp,boolDP(counter,tempraw[,i]))
        # } else {
          tmp <- rbind(tmp,inRIDP(counter,tempraw[,i]))
        # }
        counter <- counter + 1
